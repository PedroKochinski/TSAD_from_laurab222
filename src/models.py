...
class Attention(nn.Module):
	def __init__(self, feats):
		...
		self.lr = 0.0001
		self.n_feats = feats
		self.window = 5 # MHA w_size = 5
		self.n = self.n_feats * self.window
...
class LSTM_AD(nn.Module):
	def __init__(self, feats, window=None, prob=False):
...
class LSTM_AE(nn.Module):
	def __init__(self, feats, window=None, prob=False):
		...
		self.n_hidden = 64
		self.window = window
		self.lstm = nn.LSTM(input_size=self.window, hidden_size=self.n_hidden, num_layers=1, batch_first=True)
	...
	def forward(self, x):	
		...
		# 	latent = h1.repeat(self.window, 1, 1)
		...
		# outputs = outputs.view(-1, self.window, self.n_feats)
		# outputs = torch.squeeze(torch.stack(outputs))
		return outputs
	
## DAGMM Model (ICLR 18)
class DAGMM(nn.Module):
	def __init__(self, feats, window=5, prob=False):
		...
		self.n_hidden = 16
		self.n_latent = 8
		self.window = window # DAGMM w_size = 5
		self.n = self.n_feats * self.window
		self.n_gmm = self.n_feats * self.window
...
class MSCRED(nn.Module):
	...
	def forward(self, g):
		## Encode
		z = g.view(1, 1, self.n_feats, self.window)
...
# 		data = data.view(self.window, self.n_feats)
...
# 		data = data.view(self.window+1, self.n_feats, 1)
...
# 		self.window = 5
# 		self.n_hidden = 16
# 		self.n = self.window * self.n_feats
...
# 			nn.Linear(self.n_hidden, self.window), nn.Softmax(dim=0),
# 		)
# 		self.fcn = nn.Sequential(
# 			nn.Linear(self.n_feats, self.n_hidden), nn.LeakyReLU(True),
# 			nn.Linear(self.n_hidden, self.window), nn.Sigmoid(),
# 		)

	# def forward(self, data):
	# 	# Bahdanau style attention
	# 	att_score = self.attention(data).view(self.window, 1)
	# 	data = data.view(self.window, self.n_feats)
...
class MAD_GAN(nn.Module):
	def __init__(self, feats, window=None, prob=False):
		...
		self.n_feats = feats
		self.n_hidden = 16
		self.window = 5 # MAD_GAN w_size = 5
		self.n = self.n_feats * self.window
...
class TranAD_Basic(nn.Module):
	def __init__(self, feats):
		...
		self.window = 10
		self.n = self.n_feats * self.window
		self.pos_encoder = PositionalEncoding(feats, 0.1, self.window)
...
class TranAD_Transformer(nn.Module):
	def __init__(self, feats):
		...
		self.n_hidden = 8
		self.window = 10
		self.n = 2 * self.n_feats * self.window
...
class TranAD_Adversarial(nn.Module):
	def __init__(self, feats):
		...
		self.window = 10
		self.n = self.n_feats * self.window
		self.pos_encoder = PositionalEncoding(2 * feats, 0.1, self.window)
...
class TranAD(nn.Module):
	def __init__(self, feats, window, prob=False):
		super(TranAD, self).__init__()
		self.name = 'TranAD'
		self.lr = lr
		self.batch = 24 # 1 if window > 1280 else int(1280 / window) # 128
		self.n_feats = feats
		self.window = window
		self.n = self.n_feats * self.window
		self.prob = prob
...
class iTransformer(nn.Module):
	def __init__(self, feats, window, step_size=None, dmodel=2, prob=False, weighted_window=False, forecasting=False):
		super(iTransformer, self).__init__()
		self.name = 'iTransformer'
		self.weighted = weighted_window
		self.lr = lr
		self.batch = 5 if window > 1000 else int(1280 / window) # 128 for window size 10
		self.n_feats = feats
		self.window = window
		if step_size is not None:
			self.test_step_size = step_size
		else:
			self.test_step_size = window
		self.n = self.n_feats * self.window
		self.seq_len = self.window
		self.label_len = self.window
		...
		if self.forecasting:
			...
			self.pred_len = self.window
		self.output_attention = False
		self.use_norm = True
		self.d_model = dmodel  # int(self.window / 2) # * feats  # 512
		self.embed = 'TimeF'
		self.freq = 's'
		self.dropout = 0.1
...
class iTransformer_dec(nn.Module):
	def __init__(self, feats, window, prob=False):
		...
		self.lr = lr
		self.batch = 48 # if window > 1280 else int(1280 / window) # 128 for window size 10
		self.n_feats = feats
		self.window = window
		self.n = self.n_feats * self.window
		self.seq_len = self.window
